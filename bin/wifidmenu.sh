...
choice=$(echo -e "📡 Connect to network\n$wifi_toggle\n💾 Saved networks list" | dmenu -p "Select action:")

case "$choice" in
    "📡 Connect to network")
        bssid=$(nmcli -f BSSID,SSID,SIGNAL,RATE,BARS,SECURITY dev wifi list | sed -n '1!p' | dmenu -p "Select Wi-Fi: " -l 20 | awk '{print $1}')
    ...
    "💾 Saved networks list")
        ssid=$(nmcli connection show | awk -F'  +' 'NR>1 {print $1}' | dmenu -p "Select network:" -l 10)
        [ -z "$ssid" ] && exit 0  # Exit if no network selected
        
        action=$(echo -e "📋 Copy password\n🗑️ Delete network" | dmenu -p "Action:")
